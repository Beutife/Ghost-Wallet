...
export default function CreateGhostModal({
  ...
  // Removed incomplete balance line; add back with proper tuple if needed
  // Example: const balance = await publicClient.readContract({
  ...
  const handleConfirmCreate = async () => {
    ...
    try {
      ...
      ] as const; // Define ABI as const for type safety
      const logs = parseEventLogs({
        abi: ghostCreatedEvent,
        logs: receipt.logs,
      });
      ...
      const ghostAddress = event.args.ghostAddress;
      ...
      toast.error("Failed to create ghost wallet");
      setStep("form");
    }
  };
